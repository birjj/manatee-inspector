    ...
    <div className={style.wrapper}>
      <div className={style.container}>
        ...
        <div className={style.section} id="credentials">
          <h2>Credentials</h2>
          <p>
            Credentials used to communicate with the Manatee instance. Must
            match those used in Cuesta.
          </p>
          <CredentialsForm />
        </div>
        ...
        <div className={style.section} id="manatee-ports">
          ...
          <form className={style.row}>
            <label style={{ marginRight: "5ch" }}>
              <h3>Port:</h3> {port}
            </label>
            <label>
              <h3>Secure port:</h3> {securePort}
            </label>
          </form>
        </div>
      </div>
      ...
      <tbody>
        {applications.map(({ uuid, name }) => (
          <tr key={uuid}>
            ...
            <td>{uuid}</td>
            <td
              className={style["trash-cell"]}
              onClick={() => removeApplication(uuid)}
            >
              <TrashIcon />
            </td>
          </tr>
  ...
  return (
    <div {...props}>
      <form className={style.row} onSubmit={onSubmit}>
        ...
        <button
          className={username === name && password === pass ? "" : "primary"}
        >
          Save
        </button>
      </form>
  ...
  return (
    <>
      <form className={style.row} onSubmit={onSubmit}>
        ...
        <Help>
          Enter the UUID of the application you want to work on, as well as a
          human-friendly name.
          <br />
          The UUID must match the one from Cuesta.
        </Help>
        <label style={{ marginRight: "1ch", marginLeft: "1ch" }}>
          Name:
          <input
        ...
        <button disabled={disabled} className={isSubmitable ? "primary" : ""}>
          Add
        </button>
      </form>
      {error ? <div className={style.error}>{error}</div> : null}
