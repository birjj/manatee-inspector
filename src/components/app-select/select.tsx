...
const AppSelect = ({
  ...
  Partial<
    Omit<React.HTMLAttributes<HTMLDivElement>, keyof AppSelectProps>
  >) => {
  ...
  const cName = [className || "", style.select, isOpen ? style.open : ""].join(
    " "
  );
  return (
    <div {...props} className={cName} ref={$container}>
      <div
        onClick={() => setOpen(!isOpen)}
        className={[style.input, isDisabled ? style.disabled : ""].join(" ")}
      >
        <span
          className={[style.label, !selected ? style.placeholder : ""].join(
            " "
          )}
        >
        ...
        <ol className={style.menu}>
          {applications.map(({ uuid, name }) => (
            <li
              ...
              className={[
                style.item,
                uuid === value ? style["item--active"] : "",
              ].join(" ")}
              onClick={() => doSelect(uuid)}
            >
