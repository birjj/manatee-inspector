 ...
 **/
const treeFactory = <DataType, ChildProps>(
  Opener: React.ComponentType<TreeOpenerProps<DataType>>,
  Closer: React.ComponentType<TreeCloserProps<DataType>>,
  categorizer: TreeChildCategorizer<DataType, ChildProps>
) => {
  const Tree = ({
    data,
    open = false,
    ...
    const [childData, setChildData] = useState(
      [] as ReturnType<typeof categorizer>
    );
    ...
    const toggleOpen: MouseEventHandler<HTMLSpanElement> = useCallback(
      (e) => {
        e.stopPropagation();
        setOpen(!isOpen);
      },
      ...
      <div {...props} className={className}>
        <div
          className={[
            style.line,
            style["line--opener"],
            isSelected && (selected === TreeSelectState.Opener || !isOpen)
              ? style.selected
              : "",
          ].join(" ")}
          onClick={(e) => {
            e.stopPropagation();
            doSelect(TreeSelectState.Opener);
          }}
        >
          {childData.length ? (
            <span className={style.arrow} onClick={toggleOpen}>
              ▶
            </span>
          ) : null}
          ...
          <>
            ...
            <div
              className={[
                style.line,
                style["line--closer"],
                isSelected && selected === TreeSelectState.Closer
                  ? style.selected
                  : "",
              ].join(" ")}
              onClick={(e) => {
                e.stopPropagation();
                doSelect(TreeSelectState.Closer);
              }}
            >
              <div className={style["hover-adjacent"]}>
                <Closer data={data} />
