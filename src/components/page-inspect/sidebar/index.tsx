...
const DOMInspector = ({ data: _data }: DOMInspectorProps) => {
  ...
  return (
    <div
      className={[style.container, !data ? style["container--empty"] : ""].join(
        " "
      )}
    >
      {data ? (
        <>
          <div
            className={["bar", "bar--text", style["selector-bar"]].join(" ")}
          >
            <input
          ...
          <div className={style.content}>
            {Object.keys(data).map((key) => {
              return (
                <div className={style.prop} key={key}>
                  <span className={style.name}>{key}</span>:{" "}
                  <Value expanded data={data[key]} />
                </div>
              );
            })}
          </div>
