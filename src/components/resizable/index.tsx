...
const Resizable = ({
  ...
  style: styleProp,
  ...props
}: ResizableProps & Omit<React.HTMLAttributes<HTMLDivElement>, "children">) => {
  ...
  const startDrag: MouseEventHandler = useCallback(
    (e) => {
      e.preventDefault();
  ...
  const updateDrag = useCallback(
    (e: MouseEvent) => {
      ...
      setSizePercentage(ownSize ? newSize / ownSize[widthHeight] : 0.5);
    },
    [
      direction,
      startPosition,
  ...
  return (
    <div {...props} className={cName} style={styleProp} ref={$container}>
      <div
        className={[style.left, aClass || ""].join(" ")}
        ref={$a}
        style={{
          [widthHeight]:
            sizePercentage !== null ? `${sizePercentage * 100}%` : undefined,
        }}
      >
        <div
          className={style["reflow-blocker"]}
          style={{ [widthHeight]: isDragging ? `${aSize}px` : "" }}
        >
          {children[0]}
        </div>
      </div>
      <span
        className={[style.border, isDragging ? style.dragging : ""].join(" ")}
        onMouseDown={startDrag}
      />
      <div
        className={[style.right, bClass || ""].join(" ")}
        ref={$b}
        style={{
          [widthHeight]:
            sizePercentage !== null
              ? `${(1 - sizePercentage) * 100}%`
              : undefined,
        }}
      >
        <div
          className={style["reflow-blocker"]}
          style={{ [widthHeight]: isDragging ? `${bSize}px` : "" }}
        >
          {children[1]}
        </div>
      </div>
