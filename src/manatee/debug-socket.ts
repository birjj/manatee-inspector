...
export default class DebugSocket {
  ...
  closed = false;
  listeners: {
    [k in keyof DebugSocketEvents]?: ((
      ...vals: DebugSocketEvents[k]
    ) => void)[];
  } = {};
  ...
  on<T extends keyof DebugSocketEvents>(
    ev: T,
    listener: (...vals: DebugSocketEvents[T]) => void
  ) {
  ...
  off<T extends keyof DebugSocketEvents>(
    ev: T,
    listener: (...vals: DebugSocketEvents[T]) => void
  ) {
    if (!this.listeners[ev]) {
  ...
  emit<T extends keyof DebugSocketEvents>(
    ev: T,
    ...vals: DebugSocketEvents[T]
  ) {
