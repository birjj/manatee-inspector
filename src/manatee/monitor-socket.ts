...
export default class MonitorSocket {
  ...
  closed = false;
  listeners: {
    [k in keyof MonitorSocketEvents]?: ((
      ...vals: MonitorSocketEvents[k]
    ) => void)[];
  } = {};

  constructor(appId: string, port: number, securePort?: number) {
    this.url = `${securePort ? "wss" : "ws"}://localhost.sirenia.io:${
      securePort || port
    }/monitor`;
  ...
  on<T extends keyof MonitorSocketEvents>(
    ev: T,
    listener: (...vals: MonitorSocketEvents[T]) => void
  ) {
  ...
  off<T extends keyof MonitorSocketEvents>(
    ev: T,
    listener: (...vals: MonitorSocketEvents[T]) => void
  ) {
    if (!this.listeners[ev]) {
  ...
  emit<T extends keyof MonitorSocketEvents>(
    ev: T,
    ...vals: MonitorSocketEvents[T]
  ) {
